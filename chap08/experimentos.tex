...
\sigma_y^{\prime\prime} = \sigma_y - p = 0.12218|y| - 0.1 |y|
\end{equation}
...
As Figuras \ref{fig:comparacaoFinoGrosso} apresentam a comparação entre a solução do grid fino e do grosso. Pode-se notar que a solução é distorcida mas mesmo assim guarda similaridades com a solução original. Para uma avaliação quantitativa é apresentada a \ref{table:erroRelativoEngrossamento}, a primeira coluna apresenta quantos elementos finos foram utilizados para gerar um grosso. Fica claro que quanto maior o nível de engrossamento mais distante da solução original da malha fina com o engrossamento de 64x64 já mostrando uma solução bem afastada da original. 
Para o caso de engrossamento 32x32 a Figura \ref{fig:subsidence} mostra a subsidência do topo do reservatório para a solução fina e solução grossa e, pode-se constatar, uma boa aproximação da subsidência pelo grid grosso.
