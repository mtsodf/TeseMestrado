...
Inicialmente, é necessário atestar que o código de elementos finitos está resolvendo corretamente o operador descrito em \eqref{eq:edp_geomec}. \suge{Corrigindo uma sugestão que fiz anteriormente e estava errada. No caso de equações não é necessário colocar a palavra Equação, apenas o comando \Verb+eqref+. Para Tabelas, Figuras, Algoritmos e similares, deve-se colocar a palavra em maiúscula mesmo} {\color{red}{em \eqref{eq:edp_geomec}}}.
...
\caption{Gráficos do logaritmo do erro (\eqref{eq:erroAnalitico}) em função do \suge{frase incompleta}}
\end{figure}
...
\suge{apenas para comparar vou colocar a mesma tabela usando recurso do pacote booktabs, prefiro esta estética, mas o trabalho é seu, escolha a que melhor lhe pareça}
