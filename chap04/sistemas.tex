...
Define-se como número de condicionamento da matriz a equação apresentada em \eqref{eq:condicionamento}, quanto maior o número de condicionamento mais próximo da matriz ser singular \perg{de onde você tirou essa informação? Imagine uma matriz diagonal com elementos com várias ordens de grandeza de diferença nas entradas diagonais, terá um número de condicionamento alto mas não terá proximidade com uma matriz singular} \resp{Eu lembrava dessa informação dos slides da matéria do Álvaro. Fui consultar o \citet{heath1997scientific} que era o livro base da matéria e tem o parágrafo \ref{fig:condicionamento} dizendo isso.}. As matrizes advindas de sistemas gerados por \eqref{eq:edp_geomec_contorno} costumam ter números de condicionamentos elevados fazendo com que os métodos de solução tenham dificuldade de convergir.
...
Em relação as relaxações utilizadas pelo método multigrid, duas bastante utilizadas são a Jacobi e o Gauss-Seidel. Elas tem como intuito remover os erros de alta frequência em cada um dos níveis multigrid. Em particular, a relaxação de Gauss-Seidel simétrica foi utilizada na comparação do Capítulo \ref{ch:resultados} e é mostrada no Algoritmo \ref{alg:gauss_seidel}. É importante notar que, nesse caso, a quantidade de operações feitas é proporcional a duas vezes o número de não zeros da matriz , pois, a relaxação é realizada começando de i=1 e depois voltando de i=n. \perg{Aonde você viu essa definição do Gauss-Seidel simétrico}\comen{Neste algoritmo que você mostra, em cada um dos dois loops principais, todas as entradas de A entram nas contas, enquanto que no Gauss-Seidel simétrico, no primeiro loop entram apenas as da parte triangular inferior e no segundo as da parte superior. Seja A=L+D+U, onde L é a parte triangular estritamente inferior, D a diagonal e  U a triangular estritamente superior. O método simétrico de Gauss-Seidel seria:
$x^{k+1/2}=x^k+(L+D)^{-1}r^k$ e $x^{k+1}=x^{k+1/2}+(D+U)^{-1}r^{k+1/2}$, onde $r^k$ é o resíduo da iteração $k$. O livro do Saad, Iterative Methods, tem esse algoritmo explicado} \resp{Na verdade tinha retirado do código do pyamg. Coloquei uns prints \ref{fig:gauss1} e \ref{fig:gauss2} }
...
Os pré-condicionadores baseados em fatoração incompletas (ILU) apresentados em \citet{ilupaper} \comen{Essa referência está errada, o paper seminal do ILU está em  doi:10.1090/S0025-5718-1977-0438681-4} calculam uma aproximação para a fatoração LU da matriz. A aproximação é realizada desconsiderando algumas entradas das matrizes $\mathbf{L}$ e $\mathbf{U}$. Um parâmetro do método é o nível do ILU que controla a quantidade de entradas que a fatoração incompleta possui; quando o nível é  zero, a fatoração possui não zeros nas mesmas posições em que a matriz $\mathbf{A}$ possui entradas não-nulas. Há várias outras alternativas baseadas em aproximações da fatoração LU, ver~\cite{solverlinear}.
