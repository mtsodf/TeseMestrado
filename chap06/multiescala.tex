...
\end{equation}
\suge{acho que vale a pena explicar melhor esta igualdade \eqref{eq:aproximacaogrossa}. Em que sentido a solução no grid fino é aproximadamente igual à solução no grid grosso?  A do grid fino está definida em mais pontos. Talvez seja o caso de definir quem é $\msfinesolution$} \resp{Na verdade, os valores dos pontos no grid grosso são representados pelos $\freedomcoarse$. A equação \ref{eq:aproximacaogrossa} já é a função aproximada no domínio $\Omega$. No caso, assim como $u^h: \Omega \rightarrow R^2$ tem-se $u^H: \Omega \rightarrow R^2$ }
...
\perg{Aqui está sendo usada uma notação ligeiramente diferente da do Castelletto, pelo que entendi o S dele, indica o T daqui, ver eqs (A.3) e (A.5) do apêndice A do paper original. Alguma razão para essa alteração} \resp{Na verdade o Castelleto utiliza a notação tensorial enquanto que aqui a notação está toda matricial. Nesse caso o T não costuma aparecer na notação dele. Como aqui aparece acabei descendo o S para sobrescrito ao invés de ficar em cima.}
...
\omeint{ (\sopnabla \mathbf{w})^T \mathbf{D} \sopnabla  \mathbf{u}} - \int_{\Gamma_\sigma} \mathbf{w}^T \bar{\mathbf{t}} d\Gamma = (\sopnabla\mathbf{w})^T \mathbf{m} P_p \quad \forall \mathbf{w} \in \trialcoarseaprox
\end{split}
\end{empheq}
